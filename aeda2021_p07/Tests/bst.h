...
public:
    ...
    BST<Comparable> &operator=(const BST &rhs);
...
class BSTItrPost {
public:
    explicit BSTItrPost(const BST<Comparable> &bt);
...
private:
    std::stack<BinaryNode<Comparable> *> itrStack;
...
class BSTItrPre {
public:
    explicit BSTItrPre(const BST<Comparable> &bt);
...
void BSTItrPre<Comparable>::advance() {
    BinaryNode<Comparable> *current = itrStack.top();
    BinaryNode<Comparable> *next = current->left;
    if (next)
        itrStack.push(next);
    else {
...
class BSTItrIn {
public:
    explicit BSTItrIn(const BST<Comparable> &bt);
...
class BSTItrLevel {
public:
    explicit BSTItrLevel(const BST<Comparable> &bt);
...
private:
    std::queue<BinaryNode<Comparable> *> itrQueue;
...
void BSTItrLevel<Comparable>::advance() {
    BinaryNode<Comparable> *current = itrQueue.front();
    itrQueue.pop();
    BinaryNode<Comparable> *next = current->left;
    if (next)
        itrQueue.push(next);
...
public:
    const iteratorBST<Comparable> & operator++(int);
...
template<class Comparable>
const iteratorBST<Comparable> & iteratorBST<Comparable>::operator++(int) {
    BinaryNode<Comparable> *current = itrStack.top();
    itrStack.pop();
    BinaryNode<Comparable> *next = current->right;
    if (next)
        slideLeft(next);
    return *this;
}
